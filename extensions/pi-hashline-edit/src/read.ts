  ...
  pi.registerTool({
    ...
    renderResult(result, options, theme) {
      ...
      const textContent = result.content?.find(
        (entry: { type: string; text?: string }): entry is { type: "text"; text: string } =>
      ...
      const lines = text.split("\n");
      const maxLines = options.expanded ? lines.length : Math.min(lines.length, 12);
      const shown = lines.slice(0, maxLines);
      let output = shown.join("\n");
      if (lines.length > maxLines) {
        const remaining = lines.length - maxLines;
        const hint = options.expanded ? "" : " • Ctrl+O to expand";
        output += `\n${theme.fg("muted", `... (${remaining} more ${remaining === 1 ? "line" : "lines"}${hint})`)}`;
      }
