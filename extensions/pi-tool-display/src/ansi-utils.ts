 ...
 * Used as defense-in-depth to ensure no terminal control codes
 * leak into LLM context through tool results.
 */
export function stripAllAnsi(text: string): string {
  if (!text || typeof text !== "string") return text;
  // Strip SGR (Select Graphic Rendition) codes: colors, styles
  ...
  result = result.replace(/\x1b[^[\]]*[\x00-\x1f@-_~]/g, "");
  return result;
}
