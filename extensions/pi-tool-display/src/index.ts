...
import { stripAllAnsi } from "./ansi-utils.js";
import {
...
export default function toolDisplayExtension(pi: ExtensionAPI): void {
  ...
  // Defense-in-depth: strip ANSI escape sequences from tool result content
  // to prevent terminal control codes from leaking into LLM context.
  // This catches cases where a tool's execute() output contains raw ANSI
  // (e.g. from piped commands) or where rendering-layer escape sequences
  // end up in content blocks meant for the model.
  pi.on("tool_result", async (event) => {
    const output = event.output as { content?: Array<{ type: string; text?: string }> } | undefined;
    if (!output?.content) return;
    for (const block of output.content) {
      if (block.type === "text" && typeof block.text === "string") {
        block.text = stripAllAnsi(block.text);
      }
    }
  });
